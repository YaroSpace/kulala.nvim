...
local script_client = {
  ...
  { "client.assert", "client.assert", "Alias for assert" },
...
local function make_item(label, description, kind, detail, documentation, text, format, score)
  return {
    ...
    insertText = text,
    insertTextFormat = format or vim.lsp.protocol.InsertTextFormat.PlainText,
...
local function generic_source(source)
  ...
  kind = kind or vim.lsp.protocol.CompletionItemKind.Value
...
local function dynamic_variablies()
  ...
  local vars = Dynamic_variables.retrieve_all()
  local auth = { ["$auth.token"] = "Auth", ["$auth.idToken"] = "Auth" }
  local items = {}

  vim.iter(vars):each(function(name, value)
    value = type(value) == "function" and tostring(value()) or value
    table.insert(
      items,
      make_item(name, value, vim.lsp.protocol.CompletionItemKind.Variable, name, value, name:sub(2) .. "}}")
    )
  end)

  kind = vim.lsp.protocol.CompletionItemKind.Snippet
  local format = vim.lsp.protocol.InsertTextFormat.Snippet

  vim.iter(auth):each(function(name, value)
    table.insert(items, make_item(name, value, kind, name, "", name:sub(2) .. '("$1")}}$0', format))
  end)

  return items
end

local vars, reqs, buffer, lnum
local function get_document()
  if buffer == vim.fn.bufnr() and lnum == vim.fn.line(".") then return end

  vars, reqs = Parser.get_document()
...
local function request_names()
  local kind = vim.lsp.protocol.CompletionItemKind.Value
  local items = {}

  get_document()

  vim.iter(reqs):each(function(request)
...
local function variables()
  ...
  vim.iter(vars):each(function(name, value)
    table.insert(items, make_item(name, value, kind, name, value, name .. "}}"))
  end)
...
local function auth_configs()
  local kind = vim.lsp.protocol.CompletionItemKind.Variable
  local config = Oauth.get_env()
  local items = {}

  vim.iter(vim.tbl_keys(config)):each(function(name)
...
local function scripts()
  ...
  vim.list_extend(_scripts, script_client)
  vim.list_extend(_scripts, script_request)
  vim.list_extend(_scripts, script_response)
  vim.list_extend(_scripts, script_assert)
...
---@field [4] integer|nil The source kind vim.lsp.protocol.CompletionItemKind
---@field [5] integer|nil The source insert text format vim.lsp.protocol.InsertTextFormat
