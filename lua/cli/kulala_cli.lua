...
local setup = function()
  ...
  Kulala.setup(vim.tbl_deep_extend("force", opts, {
...
local function print_requests(requests)
  ...
  Colors.print(tbl:get_headers(), "Special")
  Logger.info("\n")
...
local function print_response()
  ...
  vim.cmd("so " .. kulala_path .. "/syntax/kulala_verbose_result.vim")
  vim.cmd("redraw")
...
local function run()
  ...
  Cmd.run_parser(requests, variables, nil, function()
    ...
    if args.view == "report" and not is_last() then return end
    print_response()

    if #db.responses == #requests then processing = false end
  end)
